...
describe("Rate Limiter", () => {
  it("allows requests under the limit", () => {
    ...
    const result = applyRateLimit(req, config);
  ...
  it("blocks requests over the limit", () => {
    ...
    // Use up the limit
    applyRateLimit(mockRequest(ip), config);
    applyRateLimit(mockRequest(ip), config);
    applyRateLimit(mockRequest(ip), config);

    // Fourth request should be blocked
    const result = applyRateLimit(mockRequest(ip), config);
  ...
  it("tracks different IPs separately", () => {
    ...
    applyRateLimit(mockRequest(ip1), config);
    applyRateLimit(mockRequest(ip1), config);

    // ip1 should be blocked
    expect(applyRateLimit(mockRequest(ip1), config).allowed).toBe(false);
    // ip2 should still be allowed
    expect(applyRateLimit(mockRequest(ip2), config).allowed).toBe(true);
  });
});
