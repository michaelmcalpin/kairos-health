...
/**
 * Compute a client's "today" payload for a given calendar date.
 *
 * Extracted from the `getToday` query so it can be reused by the client
 * adherence history (getAdherence) and the coach adherence dashboard
 * (coach.clients.getClientAdherence) — every caller gets the exact same
 * `{ date, advice, sections, progress }` shape, so done/total always match.
 *
 * `db` is the shared drizzle client, `userId` the client's db id, and `date`
...
export async function computeToday(
  ...
  const adherence = await safeQ(
    () =>
      db.query.adherenceLogs.findMany({
        where: and(eq(adherenceLogs.clientId, userId), eq(adherenceLogs.date, date)),
      }),
  ...
  const completions = await safeQ(
    () =>
      db.query.dailyChecklistCompletions.findMany({
        where: and(
          eq(dailyChecklistCompletions.clientId, userId),
  ...
  const appts = await safeQ(
    () =>
      db.query.appointments.findMany({
        where: and(eq(appointments.clientId, userId), eq(appointments.date, date)),
      }),
  ...
  const tasks = await safeQ(
    () =>
      db.query.clientTasks.findMany({
        where: eq(clientTasks.clientId, userId),
  ...
  const cycles = await safeQ(
    () =>
      db.query.peptideCycles.findMany({
        where: and(eq(peptideCycles.clientId, userId), eq(peptideCycles.status, "active")),
  ...
  const protocol = await safeQ(
    () =>
      db.query.supplementProtocols.findFirst({
        where: and(
          eq(supplementProtocols.clientId, userId),
  ...
  if (protocol) {
    items = await safeQ(
      () => db.query.protocolItems.findMany({ where: eq(protocolItems.protocolId, protocol.id) }),
      [] as Array<Record<string, unknown>>,
    );
  }
  ...
  const plan = await safeQ(
    () =>
      db.query.mealPlans.findFirst({
        where: and(eq(mealPlans.clientId, userId), eq(mealPlans.status, "active")),
  ...
  const assignment = await safeQ(
    () =>
      db.query.clientWorkoutAssignments.findFirst({
        where: and(
          eq(clientWorkoutAssignments.clientId, userId),
  ...
  const fastingProto = await safeQ(
    () =>
      db.query.fastingProtocols.findFirst({
        where: and(
          eq(fastingProtocols.clientId, userId),
  ...
  const adviceRows = await safeQ(
    () =>
      db.query.clientDailyAdvice.findMany({
        where: eq(clientDailyAdvice.clientId, userId),
      }),
...
/** UTC-safe "YYYY-MM-DD" + delta days → "YYYY-MM-DD" (no timezone drift). */
...
/**
 * Daily completion history for a client — the last `days` calendar dates
 * ending at `endDate`. Reused by the client-facing adherence strip and the
 * coach adherence dashboard so both read the exact same numbers.
 */
export async function computeAdherence(
  ...
  days: Array<{ date: string; done: number; total: number; pct: number | null }>;
  todayPct: number | null;
  streak: number;
}> {
  const dates: string[] = [];
  for (let i = days - 1; i >= 0; i--) dates.push(shiftDate(endDate, -i));

  // One failing day must not sink the whole response.
  const results = await Promise.all(
    dates.map(async (date) => {
      try {
        const t = await computeToday(db, userId, date);
        const done = t.progress.done;
        const total = t.progress.total;
        return { date, done, total, pct: total > 0 ? Math.round((done / total) * 100) : null };
      } catch {
        return { date, done: 0, total: 0, pct: null as number | null };
      }
    }),
  );

  const todayPct = results.length > 0 ? results[results.length - 1].pct : null;

  // Consecutive fully-complete days counting back from the most recent date.
  let streak = 0;
  for (let i = results.length - 1; i >= 0; i--) {
    const r = results[i];
    if (r.total > 0 && r.done === r.total) streak++;
    else break;
  }

  return { days: results, todayPct, streak };
}
...
export const clientTodayRouter = router({
  getToday: clientProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return computeToday(ctx.db, ctx.dbUserId, date);
    }),

  // Daily completion history + streak for the client's own adherence strip.
  getAdherence: clientProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return computeAdherence(ctx.db, ctx.dbUserId, days, endDate);
    }),
